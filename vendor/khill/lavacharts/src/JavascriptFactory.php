...
var lava$uniqueid = lava$uniqueid || { get:null, event:null, charts:{}, registeredCharts:[] };

lava$uniqueid.get = function (chartLabel) {
    var chartTypes = Object.keys(lava$uniqueid.charts),
        chart;

    if (typeof chartLabel === 'string') {
        if (Array.isArray(chartTypes)) {
            chartTypes.some(function (e) {
                if (typeof lava$uniqueid.charts[e][chartLabel] !== 'undefined') {
                    chart = lava$uniqueid.charts[e][chartLabel].chart;
        ...
        console.error('[Lavacharts] The input for lava$uniqueid.get() must be a string.');

        return false;
    }
};

lava$uniqueid.event = function (event, chart, callback) {
    return callback(event, chart);
};

lava$uniqueid.register = function(type, label) {
    this.registeredCharts.push(type+':'+label);
};

window.onload = function() {
    onResize(function() {
        for(var c = 0; c < lava$uniqueid.registeredCharts.length; c++) {
            var parts = lava$uniqueid.registeredCharts[c].split(':');
            lava$uniqueid.charts[parts[0]][parts[1]].draw();
        }
    });
};
JSCORE;
...
$out .= sprintf(
'lava'.$uniqueid.'.charts.%s = {"%s":{chart:null,draw:null,data:null,options:null,formats:[]}};',
$this->chart->type,
$this->chart->label
).PHP_EOL.PHP_EOL;
    ...
    $out .= sprintf(
        "lava$uniqueid.charts.%s.%s.draw = function() {",
        $this->chart->type,
        $this->chart->label
    ).PHP_EOL;

    $out .= sprintf(
        'var $this = lava'.$uniqueid.'.charts.%s.%s;',
        $this->chart->type,
        $this->chart->label
    ).PHP_EOL.PHP_EOL;
    ...
    $out .= sprintf(
        "google.setOnLoadCallback(lava$uniqueid.charts.%s.%s.draw);",
        $this->chart->type,
        $this->chart->label
    ).PHP_EOL;

    $out .= sprintf(
        "lava$uniqueid.register('%s', '%s');",
        $this->chart->type,
        $this->chart->label
    ).PHP_EOL;
